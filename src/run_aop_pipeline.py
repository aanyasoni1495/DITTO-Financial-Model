...
from forecast_aop import (load_cohorts, load_price_history, forecast_aop, month_index,
                           load_sheet_acquisition_forecast)
...
def run():
    ...
    for i in range(1, n_forecast_months + 1):
        ...
        forecasts[label] = forecast_aop(spells, idx, curves, price_history, sheet_acquisitions)
...
def write_report(curves, mix_m, mix_3, confidence, avg_pct, cv_results,
    ...
    with open("outputs/aop_report.md", "w") as f:
        ...
        f.write("**Note 1 (short-term oscillation):** these numbers rise and fall on a repeating "
        ...
        f.write("**Note 2 (long-term flattening):** new-signup price is fixed at August 2026's "
                "real average (£19.35 Monthly, £46.65 3-Month) for the entire forecast, per "
                "explicit instruction. Since the sheet's own acquisition forecast grows "
                "roughly 4x by 2029, new signups increasingly dominate the blend over time, "
                "pulling AOP toward this fixed price rather than growing -- this is why the "
                "forecast settles around £39-40 in later years rather than climbing. This is "
                "a direct, mechanical consequence of pinning new-signup price at August's "
                "level combined with the sheet's own growth assumption, not a bug. If actual "
                "future pricing changes, this forecast will not reflect that until re-run with "
                "an updated fixed price.\n\n")
